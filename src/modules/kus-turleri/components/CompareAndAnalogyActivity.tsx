  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="p-3 bg-white rounded-[20px] border border-stone-200 space-y-1">
          ...
          <select
            ...
            className="w-full p-2 rounded-[16px] bg-stone-50 border border-stone-200 text-xs font-semibold text-stone-900 focus:outline-none"
          >
            {TURKEY_BIRDS.map((b) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Kuş 1 */}
        <div className="bg-white rounded-[24px] border border-stone-200 overflow-hidden shadow-xs space-y-4 p-5">
          <div className="aspect-video w-full rounded-[20px] overflow-hidden bg-stone-100 relative">
            <BirdPhoto
              src={bird1.imageUrl}
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-xl font-bold font-display text-stone-900">{bird1.name}</h3>
              <p className="text-xs text-stone-500 italic">{bird1.scientificName}</p>
            </div>
            <button
              onClick={() => handlePlay(bird1.id)}
              className="p-2 rounded-[16px] bg-stone-100 hover:bg-stone-200 text-stone-700"
              title="Sesi Dinle"
            >
              <Volume2 className={`w-4 h-4 ${playingId === bird1.id ? 'text-[var(--accent)] animate-spin' : ''}`} />
            </button>
          </div>

          <div className="space-y-2 text-xs pt-2 border-t border-stone-100">
            <div className="flex items-center gap-2 p-2 bg-stone-50 rounded-[16px]">
            ...
            <div className="flex items-center gap-2 p-2 bg-stone-50 rounded-[16px]">
              ...
              <div className="flex gap-4">
                <div>
                  ...
                  <span className="font-semibold text-stone-800">{bird1.wingspan}</span>
                </div>
                <div>
                  <span className="text-stone-400 block text-[10px] uppercase font-bold">Ağırlık</span>
                  <span className="font-semibold text-stone-800">{bird1.weight}</span>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white rounded-[24px] border border-stone-200 overflow-hidden shadow-xs space-y-4 p-5">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-xl font-bold font-display text-stone-900">{bird2.name}</h3>
            ...
            <button
              ...
              title="Sesi Dinle"
            >
              <Volume2 className={`w-4 h-4 ${playingId === bird2.id ? 'text-[var(--accent)] animate-spin' : ''}`} />
            </button>
          </div>
          ...
          <div className="space-y-2 text-xs pt-2 border-t border-stone-100">
            ...
            <div className="flex items-center gap-2 p-2 bg-stone-50 rounded-[16px]">
              ...
              <div className="flex gap-4">
                <div>
                  <span className="text-stone-400 block text-[10px] uppercase font-bold">Kanat Açıklığı</span>
                  <span className="font-semibold text-stone-800">{bird2.wingspan}</span>
                </div>
                <div>
                  <span className="text-stone-400 block text-[10px] uppercase font-bold">Ağırlık</span>
                  <span className="font-semibold text-stone-800">{bird2.weight}</span>
                </div>
              </div>
            </div>
      ...
      <div className="p-4 bg-stone-50 rounded-[20px] border border-stone-200 text-xs text-stone-600 flex items-center justify-between gap-3">
        <span>
