  ...
  return (
    <div className="space-y-6">
      ...
      {activeSubTab === 'metaphor' && (
        <div className="bg-white border border-[#EBE7E0] rounded-3xl p-6 sm:p-8 shadow-sm space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {METAPHOR_DATA.elements.map((item, idx) => (
              <div
                key={item.concept}
                className="p-5 rounded-2xl bg-[#FAF8F5] border border-[#EBE7E0] space-y-2 hover:border-blue-300 transition-colors"
              >
                ...
                <div className="text-xs text-stone-800 bg-white p-3 rounded-xl border border-stone-200 mt-2">
                  <strong className="text-rose-600 block mb-0.5">Işık ve Fotoğrafa Yansıması:</strong>
                  {item.effect}
                </div>
              </div>
            ))}
      ...
      {activeSubTab === 'analogy' && (
        <div className="max-w-xl mx-auto bg-white border border-[#EBE7E0] rounded-3xl p-6 sm:p-8 shadow-sm space-y-6">
          <div className="flex items-center justify-between text-xs text-[#8A8680]">
            <span className="font-mono">ANALOJİ {analogyIndex + 1} / {ANALOGY_ITEMS.length}</span>
            <span className="font-bold text-rose-600">Disiplinlerarası Mantık</span>
          </div>

          <div className="p-6 rounded-2xl bg-[#FAF8F5] border border-[#EBE7E0] space-y-3">
            <span className="text-xs font-bold text-stone-500 uppercase tracking-wider block">
              Analoji Kalıbı:
            </span>
            <p className="text-sm sm:text-base font-bold text-[#1F1E1B] leading-relaxed">
              "{currentAnalogy.premiseA}", <span className="text-rose-600 font-extrabold">{currentAnalogy.premiseB}</span> için ne anlama geliyorsa;
            </p>
            ...
            {currentAnalogy.options.map((opt) => {
              ...
              return (
                <button
                  ...
                  className={`w-full p-4 rounded-xl border text-left text-xs font-semibold transition-all flex items-center justify-between ${style}`}
                >
                  <span>{opt}</span>
                  {analogyResult && isCorrect && <CheckCircle2 className="w-5 h-5 text-emerald-600 shrink-0" />}
                  {analogyResult && isSelected && !isCorrect && <XCircle className="w-5 h-5 text-rose-600 shrink-0" />}
                </button>
              );
            })}
          </div>
      ...
      {activeSubTab === 'webbing' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-7 bg-white border border-[#EBE7E0] rounded-3xl p-6 shadow-sm space-y-4">
              ...
              {SEMANTIC_MAP_DATA.nodes.map((node) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`text-[10px] block mt-0.5 ${isSelected ? 'text-rose-100' : 'text-[#8A8680]'}`}>
              ...
              <div className="space-y-1.5">
                {SEMANTIC_MAP_DATA.links
                  ...
                  .map((l, idx) => (
                    ...
                    >
                      <span className="font-semibold text-rose-700">{l.relationship}</span>
          ...
          <div className="lg:col-span-5 bg-[#FAF8F5] border border-[#EBE7E0] rounded-3xl p-6 flex flex-col justify-between space-y-4">
            ...
            <div className="p-4 rounded-2xl bg-white border border-[#EBE7E0] text-xs space-y-1">
