  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs animate-in fade-in duration-200">
      <div 
        ...
        className="bg-[#FBF9F5] border border-[#E2DDD3] rounded-[24px] w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200"
      >
        {/* Modal Header */}
        <div className="px-6 py-4 border-b border-[#EAE5DA] flex items-center justify-between bg-white/60">
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-[var(--accent-light)] text-[var(--accent)] uppercase tracking-wider">
              {character.category}
            </span>
        ...
        <div className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-6 p-6 rounded-[20px] bg-white border border-[#EAE5DA]">
            <div className="flex items-center gap-6">
              ...
              <button
                ...
                className="group relative w-24 h-24 sm:w-28 sm:h-28 rounded-[20px] bg-[#FAF7F2] border-2 border-[var(--accent)]/30 hover:border-[var(--accent)] flex items-center justify-center transition-all hover:scale-105 active:scale-95 shadow-xs"
              >
                <span className="text-6xl sm:text-7xl font-bold text-[#1F1E1D] select-none">
                  {currentKana}
                </span>
                <div className="absolute -bottom-2 -right-2 p-1.5 rounded-full bg-[var(--accent)] text-white shadow-md group-hover:scale-110 transition-transform">
              ...
              <div>
                <div className="flex items-baseline gap-2">
                  <span className="text-3xl sm:text-4xl font-black text-[#1F1E1D] ">
                    {character.romaji}
                  </span>
                ...
                <div className="mt-2 flex items-center gap-1.5 text-xs text-[var(--accent)] font-semibold">
                  <span>Sesletim:</span>
            ...
            <button
              ...
              className="w-full sm:w-auto px-4 py-2.5 rounded-[16px] bg-[#1F1E1D] hover:bg-neutral-800 text-white text-xs sm:text-sm font-semibold flex items-center justify-center gap-2 transition-colors shadow-xs"
            >
          ...
          <div className="p-4 rounded-[20px] bg-[var(--accent-light)]/70 border border-[var(--accent)]/80">
            <h4 className="text-xs font-bold text-[var(--accent)] uppercase tracking-wider mb-1 flex items-center gap-1.5">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {character.sampleWords.map((sample, idx) => (
                  <div
                    key={idx}
                    className="p-3 rounded-[16px] bg-white border border-[#EAE5DA] flex items-center justify-between hover:border-[var(--accent)]/30 transition-colors"
                  >
                    <div>
                      <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="p-2 rounded-lg text-[#7A756D] hover:text-[var(--accent)] hover:bg-[var(--accent-light)] transition-colors"
                    >
                      <Volume2 className="w-4 h-4" />
                    </button>
                  </div>
          ...
          <div className="pt-2 border-t border-[#EAE5DA]">
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                ...
                { level: 1 as const, label: 'Öğreniliyor', color: 'border-[var(--accent)]/30 hover:bg-[var(--accent-light)] text-[var(--accent)]' },
                { level: 2 as const, label: 'İyi Durumda', color: 'border-[var(--accent)]/30 hover:bg-[var(--accent-light)] text-[var(--accent)]' },
                { level: 3 as const, label: 'Ustalaşıldı', color: 'border-[var(--accent)]/30 hover:bg-[var(--accent-light)] text-[var(--accent)]' },
              ].map((item) => (
                <button
                  key={item.level}
                  id={`btn-mastery-level-${item.level}`}
                  onClick={() => onSetMastery(item.level)}
                  className={`px-2 py-2 rounded-[16px] text-xs font-semibold border transition-all text-center flex flex-col items-center justify-center gap-1 ${
